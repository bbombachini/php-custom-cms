<?php
  ...
  function addMovie($cover, $title, $year, $story, $trailer, $rating, $genre) {
    include('connect.php');
    if($cover['type'] == 'image/jpg' || $cover['type'] == 'image/jpeg' && $trailer['type'] == 'video/mp4'){
      $targetpath = "../public/images/{$cover['name']}";
      $videopath = "../public/videos/{$trailer['name']}";
      if(move_uploaded_file($trailer['tmp_name'], $videopath)){
